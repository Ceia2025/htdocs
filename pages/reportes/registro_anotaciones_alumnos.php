...
?>
<?php
ob_start(); // Comienza el almacenamiento en el buffer de salida
...
class PDF extends FPDF
{
    ...
    function ShowTable($header, $data)
    {
        ...
        foreach ($header as $col) {
            $maxWidths[] = $this->GetStringWidth($col);
        }
        foreach ($data as $row) {
            foreach ($row as $key => $col) {
                $colWidth = $this->GetStringWidth($col);
        ...
        foreach ($header as $key => $col) {
            $this->Cell($maxWidths[$key] + 2, 7, $col, 1);
        }
        $this->Ln();
        // Datos
        foreach ($data as $row) {
            foreach ($row as $key => $col) {
                $this->Cell($maxWidths[$key] + 2, 6, $col, 1);
            }
            $this->Ln();
        }
    }
...
if ($result_alum && $result_alum->num_rows > 0) {
    $data_alum = $result_alum->fetch_assoc();
} else {
    // Si no hay resultados, definir un array vacío para evitar el error
    $data_alum = array();
}
