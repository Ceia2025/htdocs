...
.header-logos img {
    width: 180px; /* Puedes ajustar el tamaño si deseas que los logos sean más grandes o pequeños */
    height: auto;
}

/* Ajuste de texto y ancho completo */
.text-container, .tabla-notas {
    width: 100%;
    text-align: center;
}
...
.tabla-notas th, .tabla-notas td {
    ...
    .nota-input {
    width: 50px; /* Ajusta el tamaño de la celda según lo necesites */
...
.nota-input.semestre-1 {
    background-color: #e9f7fa; /* Color diferente para las notas del primer semestre */
}

.nota-input.semestre-2 {
    background-color: #f9f7e9; /* Color diferente para las notas del segundo semestre */
}

.promedio-1s, .promedio-2s, .promedio-final {
    background-color: #f0f0f0; /* Fondo para los promedios */
    text-align: center;
    font-weight: bold; /* Hace que los promedios se vean más destacados */
}
    ...
    .small-text {
        font-size: 15px; /* Tamaño de la fuente ajustable */
        line-height: 1;  /* Espacio mínimo entre líneas */
...
.tabla-asistencia th {
    ...
    .table-responsive table {
        width: 100%;
        max-width: 600px; /* Ajusta el ancho de la tabla para que sea más estrecha */
    ...
    .tabla-asistencia td,
    .table-responsive td {
        padding: 4px 8px; /* Reducir el espacio interno de las celdas */
    ...
    margin: 20px 0; /* Agrega un espacio vertical entre el texto y otros elementos */
...
while ($row_asignatura = $result_asignaturas->fetch_assoc()) {
    ...
    if ($num_notas_1s > 0) $promedio_asignatura_1s /= $num_notas_1s;
    if ($num_notas_2s > 0) $promedio_asignatura_2s /= $num_notas_2s;

    // Guardar los promedios de cada asignatura en el array de promedios
    if ($num_notas_1s > 0) $promedios_1s[] = $promedio_asignatura_1s;
    if ($num_notas_2s > 0) $promedios_2s[] = $promedio_asignatura_2s;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['curso']) && !empty($_POST['alumno'])) {
    ...
    if ($result_anotaciones = $conn->query($sql_anotaciones)) {
        while ($row_anotacion = $result_anotaciones->fetch_assoc()) {
            if ($row_anotacion['tipo'] == 'P') $positivas = $row_anotacion['cantidad'];
            elseif ($row_anotacion['tipo'] == 'N') $negativas = $row_anotacion['cantidad'];
            else $otras = $row_anotacion['cantidad'];
        }
    }
