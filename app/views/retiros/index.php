...
<div class="min-h-full">
    ...
    <header class="bg-gray-800 border-b border-white/10">
        <div class="mx-auto max-w-6xl px-4 py-5 sm:px-6 flex items-center justify-between flex-wrap gap-3">
            <div>
                <p class="text-xs font-semibold uppercase tracking-widest text-indigo-400 mb-0.5">Convivencia Escolar</p>
                ...
                <a href="index.php?action=retiros_reportes"
                   class="flex items-center gap-2 text-sm text-gray-300 hover:text-white border border-gray-600 hover:border-gray-400 px-4 py-2 rounded-lg transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6M4 20h16M4 4h16M9 4v4m6-4v4"/>
                    </svg>
                    Reportes
                </a>
                <?php if ($puedeEditar): ?>
                <a href="index.php?action=retiros_create"
                   class="flex items-center gap-2 text-sm font-semibold bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <main class="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        ...
        <?php if (isset($_GET['creado'])): ?>
            <div class="mb-5 flex items-center gap-2 bg-green-900/40 border border-green-700/50 text-green-300 px-4 py-3 rounded-xl text-sm font-medium">
        ...
        <?php elseif (isset($_GET['editado'])): ?>
            <div class="mb-5 flex items-center gap-2 bg-blue-900/40 border border-blue-700/50 text-blue-300 px-4 py-3 rounded-xl text-sm font-medium">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                Retiro actualizado correctamente.
            </div>
        <?php elseif (isset($_GET['eliminado'])): ?>
            <div class="mb-5 flex items-center gap-2 bg-red-900/40 border border-red-700/50 text-red-300 px-4 py-3 rounded-xl text-sm font-medium">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                Retiro eliminado.
            </div>
        <?php endif; ?>
        ...
        <form method="GET" action="index.php" class="bg-gray-800 border border-gray-700 rounded-xl p-4 mb-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-3">
                ...
                <select name="anio_id" class="bg-gray-900 text-white text-sm border border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <option value="">Todos los años</option>
                    <?php foreach ($anios as $a): ?>
                        <option value="<?= $a['id'] ?>" <?= ($_GET['anio_id'] ?? '') == $a['id'] ? 'selected' : '' ?>><?= $a['anio'] ?></option>
                    <?php endforeach; ?>
                </select>

                <select name="semestre" class="bg-gray-900 text-white text-sm border border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <select name="curso_id" class="bg-gray-900 text-white text-sm border border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <option value="">Todos los cursos</option>
                    <?php foreach ($cursos as $c): ?>
                        <option value="<?= $c['id'] ?>" <?= $cursoIdActivo == $c['id'] ? 'selected' : '' ?>><?= htmlspecialchars($c['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>

                <select name="justificado" class="bg-gray-900 text-white text-sm border border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <option value="">Justificado / No</option>
                    <option value="Si" <?= ($_GET['justificado'] ?? '') === 'Si' ? 'selected' : '' ?>>Justificado</option>
                    <option value="No" <?= ($_GET['justificado'] ?? '') === 'No' ? 'selected' : '' ?>>No justificado</option>
                </select>

                <select name="extraordinario" class="bg-gray-900 text-white text-sm border border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <option value="">Ordinario / Extraordinario</option>
                    <option value="Si" <?= ($_GET['extraordinario'] ?? '') === 'Si' ? 'selected' : '' ?>>Extraordinario</option>
                    <option value="No" <?= ($_GET['extraordinario'] ?? '') === 'No' ? 'selected' : '' ?>>Ordinario</option>
                </select>
            </div>
            <div class="flex gap-2">
                <button type="submit" class="bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold px-4 py-2 rounded-lg transition flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2a1 1 0 01-.293.707L13 13.414V19a1 1 0 01-.553.894l-4 2A1 1 0 017 21v-7.586L3.293 6.707A1 1 0 013 6V4z"/></svg>
                    Filtrar
                </button>
                <a href="index.php?action=retiros" class="text-xs text-gray-500 hover:text-gray-300 underline transition flex items-center">Limpiar</a>
            </div>
        </form>
                ...
                <div class="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
                    <div class="px-5 py-3.5 border-b border-gray-700 flex items-center justify-between">
                        <h2 class="text-sm font-semibold uppercase tracking-wider text-gray-400">
                            Detalle <span class="ml-1 text-xs font-normal text-gray-600 normal-case">(<?= count($retiros) ?> registros)</span>
                        </h2>
                        ...
                        <table class="w-full text-sm" id="tabla-retiros">
                            <thead>
                                <tr class="bg-gray-900/50">
                                    ...
                                    <th class="px-3 py-2.5 text-left   text-xs font-semibold uppercase tracking-wider text-gray-500">Curso</th>
                                    <th class="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-gray-500">Fecha / Hora</th>
                                    <th class="px-3 py-2.5 text-left   text-xs font-semibold uppercase tracking-wider text-gray-500">Motivo</th>
                                    <th class="px-3 py-2.5 text-left   text-xs font-semibold uppercase tracking-wider text-gray-500">Quien retira</th>
                                    <th class="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-gray-500">Estado</th>
                                    <?php if ($puedeEditar): ?>
                            ...
                            <tbody id="tbody-retiros">
                                ...
                                <tr class="fila-retiro border-t border-gray-700/60 hover:bg-gray-700/20 transition"
                                    ...
                                    <td class="px-4 py-3">
                                        <p class="text-white font-semibold leading-tight"><?= htmlspecialchars($r['apepat'] . ' ' . $r['apemat']) ?></p>
                                        <p class="text-gray-400 text-xs"><?= htmlspecialchars($r['nombre']) ?></p>
                                        <p class="text-gray-600 text-xs font-mono"><?= htmlspecialchars($r['run']) ?></p>
                                    </td>
                                    <td class="px-3 py-3 text-xs text-gray-400"><?= htmlspecialchars($r['curso']) ?></td>
                                    <td class="px-3 py-3 text-center">
                                        <span class="text-gray-300 text-xs block font-mono"><?= date('d/m/Y', strtotime($r['fecha_retiro'])) ?></span>
                                        <span class="text-indigo-400 font-bold font-mono"><?= substr($r['hora_retiro'], 0, 5) ?></span>
                                    ...
                                    <td class="px-3 py-3 text-xs text-gray-300 max-w-[140px]">
                                        ...
                                        <?php if ($r['observacion']): ?>
                                            <span class="block text-gray-600 italic truncate"><?= htmlspecialchars($r['observacion']) ?></span>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="px-3 py-3 text-center">
                                        <div class="flex flex-col items-center gap-1">
                                            <?php if ($r['justificado'] === 'Si'): ?>
                                                <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-green-900/50 text-green-400 border border-green-800/50">Justificado</span>
                                            <?php else: ?>
                                                <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-red-900/40 text-red-400 border border-red-800/40">No justif.</span>
                                            <?php endif; ?>
                                            <?php if ($r['extraordinario'] === 'Si'): ?>
                                                <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-900/40 text-amber-400 border border-amber-800/40">Extraord.</span>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                    <?php if ($puedeEditar): ?>
                                    <td class="px-3 py-3 text-right">
                                        <div class="flex items-center justify-end gap-1">
                                            <a href="index.php?action=retiros_edit&id=<?= $r['id'] ?>"
                                               class="text-gray-500 hover:text-indigo-400 transition p-1" title="Editar">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536M9 13l6.586-6.586a2 2 0 112.828 2.828L11.828 15.828a4 4 0 01-1.414.828l-3 1 1-3a4 4 0 01.828-1.414z"/>
                                                </svg>
                                            </a>
                                            <button type="button" onclick="confirmarEliminar(<?= $r['id'] ?>)"
                                                    class="text-gray-500 hover:text-red-400 transition p-1" title="Eliminar">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="xl:col-span-1">
                <div class="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
                    <div class="px-4 py-3 border-b border-gray-700">
                        <h3 class="text-xs font-semibold uppercase tracking-wider text-gray-400">Top alumnos con más retiros</h3>
                    </div>
                    <div class="divide-y divide-gray-700/50">
                        ...
                        <div class="flex items-center gap-3 px-4 py-2.5">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-white text-xs font-semibold truncate"><?= htmlspecialchars($al['apepat'] . ' ' . $al['apemat'] . ', ' . $al['nombre']) ?></p>
                                <p class="text-xs text-gray-500"><?= htmlspecialchars($al['curso']) ?></p>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <span class="text-base font-bold <?= $al['total'] >= 5 ? 'text-red-400' : ($al['total'] >= 3 ? 'text-amber-400' : 'text-gray-300') ?>"><?= $al['total'] ?></span>
...
<!-- Modal top alumnos -->
<div id="modalTopRetiros" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
    <div class="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-md max-h-[80vh] flex flex-col shadow-xl">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-700 flex-shrink-0">
            ...
            <button onclick="document.getElementById('modalTopRetiros').classList.add('hidden')" class="text-gray-400 hover:text-white transition">
...
<div id="modalEliminar" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
    <div class="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-sm shadow-xl p-6">
        ...
        <form id="formEliminar" method="POST" action="">
            <div class="flex gap-3 justify-end">
                <button type="button" onclick="cerrarModalEliminar()" class="rounded-lg bg-gray-700 px-4 py-2 text-sm text-gray-300 hover:bg-gray-600 transition">Cancelar</button>
                <button type="submit" class="rounded-lg bg-red-600 px-4 py-2 text-sm text-white hover:bg-red-500 transition">Eliminar</button>
            </div>
        </form>
    </div>
</div>
