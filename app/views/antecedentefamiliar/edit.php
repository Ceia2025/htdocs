...
<div class="min-h-full">
    ...
    <header class="relative bg-gray-800 after:pointer-events-none after:absolute after:inset-x-0 after:inset-y-0 after:border-y after:border-white/10">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold tracking-tight text-white">Editar Antecedente Familiar</h1>
        </div>
    </header>
        ...
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-center">
                <div class="bg-gray-700 p-6 rounded-2xl shadow-lg w-full max-w-lg">
                    <h2 class="text-2xl font-bold text-white mb-6 text-center">Actualizar Registro</h2>

                    <form method="post" action="index.php?action=antecedentefamiliar_update&id=<?= $antecedente['id'] ?>" class="space-y-4">
                        <!-- Alumno -->
                        <div>
                            ...
                            <select name="alumno_id" id="alumno_id" required
                                ...
                                <?php foreach ($alumnos as $a): ?>
                                    <option value="<?= $a['id'] ?>" <?= $a['id'] == $antecedente['alumno_id'] ? 'selected' : '' ?>>
                        ...
                        <div>
                            ...
                            <select name="padre" id="padre" class="mt-2 w-full rounded-lg bg-gray-800 border border-gray-700 text-white px-4 py-2">
                                <?php foreach ($niveles as $nivel): ?>
                                    <option value="<?= $nivel ?>" <?= $nivel == $antecedente['padre'] ? 'selected' : '' ?>><?= $nivel ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="madre" id="madre" class="mt-2 w-full rounded-lg bg-gray-800 border border-gray-700 text-white px-4 py-2">
                                <?php foreach ($niveles as $nivel): ?>
                                    <option value="<?= $nivel ?>" <?= $nivel == $antecedente['madre'] ? 'selected' : '' ?>><?= $nivel ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
