...
<div class="min-h-full">
    ...
    <header class="relative bg-gray-800 after:pointer-events-none after:absolute after:inset-x-0 after:inset-y-0 after:border-y after:border-white/10">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold tracking-tight text-white">Editar Categorización</h1>
        </div>
    </header>
        ...
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-center">
                <div class="bg-gray-700 p-6 rounded-2xl shadow-lg w-full max-w-md">
                    <h2 class="text-2xl font-bold text-white mb-6 text-center">Actualizar Categorización</h2>

                    <?php if (isset($_GET['error']) && $_GET['error'] === 'duplicado'): ?>
                        <div class="mb-4 p-3 bg-red-600 text-white rounded-lg text-center">
                            ⚠️ Ya existe una categorización con ese código general y específico.
                        </div>
                    <?php endif; ?>

                    <form method="post" action="index.php?action=categorizacion_update&id=<?= $categorizacion['id'] ?>" class="space-y-4">

                        <div>
                            <label for="nombre" class="block text-sm font-medium text-gray-200">Nombre</label>
                            <input type="text" name="nombre" id="nombre" value="<?= htmlspecialchars($categorizacion['nombre']) ?>" required
                                   ...
                                   autocomplete="off" oninput="buscarNombre(this.value)">
                            <div id="sugerencias" class="bg-gray-800 rounded-lg mt-1 shadow-lg hidden"></div>
                        </div>

                        <div>
                            <label for="codigo_general" class="block text-sm font-medium text-gray-200">Código General</label>
                            <input type="text" name="codigo_general" id="codigo_general" value="<?= htmlspecialchars($categorizacion['codigo_general']) ?>" required
                                   class="mt-2 w-full rounded-lg bg-gray-800 border border-gray-700 text-white px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        </div>

                        <div>
                            <label for="codigo_especifico" class="block text-sm font-medium text-gray-200">Código Específico</label>
                            <input type="number" name="codigo_especifico" id="codigo_especifico" value="<?= htmlspecialchars($categorizacion['codigo_especifico']) ?>" required
                                   class="mt-2 w-full rounded-lg bg-gray-800 border border-gray-700 text-white px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        </div>
...
<script>
    ...
    async function buscarNombre(termino) {
        ...
        const response = await fetch(`${baseURL}buscar_categorizacion.php?term=${encodeURIComponent(termino)}`);
