    ...
    <div class="min-h-full">
        ...
        <header class="bg-gray-800 border-b border-white/10">
            <div class="mx-auto max-w-6xl px-4 py-5 sm:px-6 flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <h1 class="text-2xl font-bold text-white">Reporte de Alumnos Retirados</h1>
                    <p class="text-xs text-gray-500 mt-0.5">Retiros por curso, con fecha y porcentaje</p>
                </div>
        ...
        <main class="mx-auto max-w-6xl px-4 py-6 sm:px-6">
            ...
            <form method="GET" action="index.php" class="bg-gray-800 border border-gray-700 rounded-xl p-4 mb-6">
                <input type="hidden" name="action" value="reportes_retirados">
                ...
                <a href="index.php?action=reportes_retirados"
            ...
            <?php if ($anioId): ?>
                ...
                <div class="flex flex-wrap gap-3 mb-6">
                    <a href="index.php?action=reportes_retirados_pdf&<?= $queryBase ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-red-700 hover:bg-red-600
            ...
            <?php if ($anioId): ?>
                <div class="bg-gray-800 border border-gray-700 rounded-xl p-5 mb-6 flex flex-wrap items-center gap-6">
                    ...
                    <div>
                        <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Retirados</p>
                        <p class="text-2xl font-bold text-red-400"><?= $resumenGlobal['total_retirados'] ?></p>
                    </div>
                    <div class="w-px h-10 bg-gray-700"></div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">% de retiro general</p>
                            ...
                            <tbody>
                                <?php foreach ($resumenCursos as $rc): ?>
                                    <tr class="border-t border-gray-700/60 hover:bg-gray-700/20 transition">
                                        <td class="px-4 py-2.5 text-white font-semibold"><?= htmlspecialchars($rc['curso']) ?>
                                        </td>
                                        <td class="px-4 py-2.5 text-center text-gray-300"><?= $rc['total_matriculados'] ?></td>
                                        <td class="px-4 py-2.5 text-center text-red-400 font-semibold">
                                            <?= $rc['total_retirados'] ?></td>
                                        <td class="px-4 py-2.5 text-center">
                                            <span
                                                class="font-bold <?= $rc['porcentaje'] >= 15 ? 'text-red-400' : 'text-cyan-400' ?>">
                                                <?= $rc['porcentaje'] ?>%
                                            </span>
                                        </td>
                                    </tr>
                                    ...
                                    <?php foreach ($alumnos as $al): ?>
                                        <tr class="border-t border-gray-700/60 hover:bg-gray-700/20 transition">
                                            ...
                                            <td class="px-3 py-2.5 text-center">
                                                <span
                                                    class="inline-flex items-center gap-1.5 text-red-400 font-semibold text-xs
                                                         bg-red-900/20 border border-red-800/40 px-2 py-1 rounded-full">
            ...
            <?php elseif ($anioId): ?>
                <div class="bg-gray-800 border border-gray-700 rounded-xl px-6 py-16 text-center">
                    ...
                    <p class="text-gray-400 font-medium">No hay alumnos retirados con los filtros seleccionados</p>
                </div>
            <?php else: ?>
                <div class="bg-gray-800 border border-gray-700 rounded-xl px-6 py-16 text-center">
                    <span class="text-5xl block mb-4">📊</span>
