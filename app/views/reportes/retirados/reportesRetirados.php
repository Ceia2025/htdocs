...
<main class="mx-auto max-w-6xl px-4 py-6 sm:px-6">
    ...
    <?php if ($anioId): ?>
        ...
        <div class="flex flex-wrap gap-3 mb-6">
            <a href="index.php?action=reportes_retirados_pdf&<?= $queryBase ?>"
    ...
    <?php if ($anioId): ?>
        <div class="panel rounded-xl p-5 mb-6 flex flex-wrap items-center gap-6">
            ...
            <div>
                <p class="text-xs text-muted uppercase tracking-wider mb-1">Retirados</p>
                <p class="text-2xl font-bold text-danger"><?= $resumenGlobal['total_retirados'] ?></p>
            </div>
            <div class="w-px h-10" style="background: var(--border-soft);"></div>
            <div>
                <p class="text-xs text-muted uppercase tracking-wider mb-1">% de retiro general</p>
                <p class="text-2xl font-bold text-accent"><?= $resumenGlobal['porcentaje'] ?>%</p>
            </div>
        </div>
    <?php endif ?>
                ...
                <table class="data-table w-full text-sm">
                    ...
                    <tbody>
                        <?php foreach ($resumenCursos as $rc): ?>
                            <tr>
                                <td class="px-4 py-2.5 text-strong font-semibold"><?= htmlspecialchars($rc['curso']) ?>
                                </td>
                            ...
                            <?php foreach ($alumnos as $al): ?>
                                <tr>
                                    ...
                                    <td class="px-3 py-2.5 text-center">
                                        <span
                                            class="chip-danger inline-flex items-center gap-1.5 font-semibold text-xs px-2 py-1 rounded-full">
    ...
    <?php elseif ($anioId): ?>
        <div class="panel rounded-xl px-6 py-16 text-center">
            ...
            <p class="text-muted font-medium">No hay alumnos retirados con los filtros seleccionados</p>
        </div>
    <?php else: ?>
        <div class="panel rounded-xl px-6 py-16 text-center">
            <span class="text-5xl block mb-4">📊</span>
