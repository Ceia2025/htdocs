    ...
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <form method="GET" action="index.php" id="formFiltros"
            ...
            <div class="flex flex-col justify-end">
                ...
                <?php else: ?>
                    <button disabled class="text-center bg-gray-700 text-gray-500 font-semibold rounded-xl px-4 py-3 cursor-not-allowed">
                        ➕ Nueva Anotación
                    </button>
                <?php endif ?>
            </div>
            ...
            <div class="overflow-x-auto bg-gray-900 rounded-3xl shadow-lg">
                <table class="min-w-full divide-y divide-gray-700">
                    <thead class="bg-gray-950/50">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-200 uppercase tracking-wider">Alumno</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-200 uppercase tracking-wider">Total</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-green-400 uppercase tracking-wider">Positivas</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-yellow-400 uppercase tracking-wider">Leves</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-orange-400 uppercase tracking-wider">Graves</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-red-400 uppercase tracking-wider">Gravísimas</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-200 uppercase tracking-wider">Acciones</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-700">
                        <?php foreach ($alumnos as $al): ?>
                            ...
                            if (!empty($_GET['semestre'])) $urlVer .= "&semestre={$_GET['semestre']}";
                            ?>
                            <tr class="bg-gray-900/20 hover:bg-gray-700/50 transition cursor-pointer"
                                ...
                                <td class="px-4 py-3 text-center">
                                    <?php if ($al['positivas'] > 0): ?>
                                        <span class="inline-flex items-center justify-center w-7 h-7 bg-green-900/40 border border-green-500 text-green-300 rounded-full text-xs font-bold"><?= $al['positivas'] ?></span>
                                    <?php else: ?><span class="text-gray-600">—</span><?php endif ?>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <?php if ($al['leves'] > 0): ?>
                                        <span class="inline-flex items-center justify-center w-7 h-7 bg-yellow-900/40 border border-yellow-500 text-yellow-300 rounded-full text-xs font-bold"><?= $al['leves'] ?></span>
                                    <?php else: ?><span class="text-gray-600">—</span><?php endif ?>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <?php if ($al['graves'] > 0): ?>
                                        <span class="inline-flex items-center justify-center w-7 h-7 bg-orange-900/40 border border-orange-500 text-orange-300 rounded-full text-xs font-bold"><?= $al['graves'] ?></span>
                                    <?php else: ?><span class="text-gray-600">—</span><?php endif ?>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <?php if ($al['gravisimas'] > 0): ?>
                                        <span class="inline-flex items-center justify-center w-7 h-7 bg-red-900/40 border border-red-500 text-red-300 rounded-full text-xs font-bold"><?= $al['gravisimas'] ?></span>
                                    <?php else: ?><span class="text-gray-600">—</span><?php endif ?>
                                </td>
